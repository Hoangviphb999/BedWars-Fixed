...
class Game
{
    ...
    public function join(Player $player) : void{
        ...
        \BedWars\utils\Scoreboard::setLine($player, 8, " " . TextFormat::WHITE . "Version: " . TextFormat::GRAY . "v1.5" . str_repeat(" ", 3));
        \BedWars\utils\Scoreboard::setLine($player, 9, "    ");
        \BedWars\utils\Scoreboard::setLine($player, 10, " " . TextFormat::YELLOW . "CRAFTOMLET");
    }
    ...
    public function killPlayer(Player $player) : void{
        ...
        if(!$playerTeam->hasBed()){
            ...
            $player->addTitle(TextFormat::BOLD . TextFormat::RED . "YOU DIED!", TextFormat::GRAY . "You won't winning on this time!");
        }else{
            $player->setGamemode(Player::SPECTATOR);
         ...
         switch($this->state) {
             case self::STATE_LOBBY;
                 if ($this->starting) {
                     ...
                     foreach ($this->players as $player) {
                         $player->sendTip(TextFormat::YELLOW . "§lBedWars §r§eStarting in " . TextFormat::AQUA . gmdate("i:s", $this->startTime));
                     }
             ...
             case self::STATE_RUNNING;
                 ...
                 foreach (array_merge($this->players, $this->spectators) as $player) {
                     ...
                     foreach ($this->teams as $team) {
                         $status = "";
                         if ($team->hasBed()) {
                             $status = TextFormat::GREEN . "§l✓";
                         } elseif(count($team->getPlayers()) < $team->dead) {
                             $status = count($team->getPlayers()) === 0 ? TextFormat::RED . "§l✗" : TextFormat::GRAY . " " . count($team->getPlayers()) . " ";
                         }elseif(count($team->getPlayers()) >= $team->dead){
                     ...
                     \BedWars\utils\Scoreboard::setLine($player, " " . $currentLine++, " " . TextFormat::YELLOW . "CRAFTOMLET");
                 }
